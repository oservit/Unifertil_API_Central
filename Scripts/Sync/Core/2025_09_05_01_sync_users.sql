...
DROP TABLE api_users CASCADE CONSTRAINTS;
...
CREATE TABLE sync_nodes (
    ...
    auth_endpoint  VARCHAR2(500) DEFAULT '/Auth/GetToken' NOT NULL,
...
CREATE TABLE api_users (
    ...
    CONSTRAINT fk_api_users_type FOREIGN KEY (type_id) REFERENCES sync_user_types(id)
);
...
CREATE TABLE sync_routes (
    ...
    description     VARCHAR2(200), -- pode ser NULL
    is_active       NUMBER(1) DEFAULT 1 NOT NULL,
    created_at      TIMESTAMP DEFAULT SYSTIMESTAMP,
    updated_at      TIMESTAMP,
    ...
    CONSTRAINT fk_sync_routes_user   FOREIGN KEY (user_id)        REFERENCES api_users(id),
...
-- View de rotas
-- ============================================
CREATE OR REPLACE VIEW sync_routes_users_view AS
SELECT 
    r.id               AS route_id,
    r.description      AS route_description,
    sns.id             AS source_node_id,
    sns.name           AS source_node_name,
    sns.url            AS source_node_url,
    snt.id             AS target_node_id,
...
FROM sync_routes r
JOIN sync_nodes sns ON r.source_node_id = sns.id
JOIN sync_nodes snt ON r.target_node_id = snt.id
JOIN api_users u  ON r.user_id = u.id
ORDER BY sns.name, snt.name;


CREATE OR REPLACE VIEW sync_scheduled_events_view AS
SELECT
    sse.id                 AS id,
    sse.entity_id          AS entity_id,
    sse.record_id          AS record_id,
    sse.operation_id       AS operation_id,
    sse.status_id          AS status_id,
    sse.payload            AS payload,
    sse.hash_value         AS hash_value,
    tn.url || tn.auth_endpoint     AS auth_endpoint,
    tn.url || sse.service_endpoint AS service_endpoint,
    u.username             AS target_username,
    u.password             AS target_password
FROM sync_scheduled_events sse
JOIN sync_routes sre
    ON (sse.source_node_id = sre.source_node_id
    AND sse.target_node_id = sre.target_node_id
    AND sre.is_active = 1)
JOIN sync_nodes tn
    ON (sse.target_node_id = tn.id)
JOIN api_users u
    ON (sre.user_id = u.id);
